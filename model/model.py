...
class GPT(nn.Module):
    ...
    def estimate_mfu(self, fwdbwd_per_iter, dt):
        """ estimate model flops utilization (MFU) in units of this machine's
        real GPU peak FLOPS, not nanoGPT's original A100 reference. Original
        code always divided by 312e12 (A100 bfloat16 peak) regardless of the
        GPU actually running, so on this RTX 4070 SUPER it reported ~12% when
        the real utilization against this card's own peak is ~53%, a units
        mismatch, not an actual performance problem. 70.96e12 is the RTX 4070
        SUPER's real dense FP16/bfloat16 tensor peak (WareDB hardware specs,
        NVIDIA does not publish this figure directly for GeForce cards).
        """
        ...
        # express our flops throughput as ratio of this GPU's own bfloat16 peak flops
        flops_achieved = flops_per_iter * (1.0/dt) # per second
        flops_promised = 70.96e12 # RTX 4070 SUPER dense bfloat16/FP16 tensor peak
